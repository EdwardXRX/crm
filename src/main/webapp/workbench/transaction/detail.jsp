<%@ page import="com.edward.crm_ssh.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.edward.crm_ssh.workbench.domain.Tran" %>
...
<div style="position: relative; left: 40px; top: -50px;">
    ...
    <%
        ...
        //判断当前阶段可能性为0
        //前七个一定是黑圈，后两个一个是红叉，一个是黑叉
        if ("0".equals(currentPossibility)) {
            for (int i = 0; i < dvList.size(); i++) {
                //取得每一个遍历出来得阶段，根据每一个遍历出来得阶段获取其可能性
                DicValue dv = dvList.get(i);
                String listStage = dv.getValue();
                String listPossibility = pMap.get(listStage);

                //如果遍历出来得阶段得可能性为0,说明是后两个
    ...
    //如果当前阶段得可能性不为0，前七个可能是绿圈，绿色标记，黑圈，后两个一定是黑叉
    else {
        //准备当前阶段的下标
        int index = 0;
        for (int i = 0; i < dvList.size(); i++) {
...
<div style="position: relative; top: 0px;">
    <div style="position: relative; left: 40px; height: 30px;">
        ...
        <div style="width: 300px;position: relative; left: 650px; top: -60px;"><b>5,${t.money}</b></div>
    ...
    <div style="position: relative; left: 40px; height: 30px; top: 70px;">
        ...
        <div style="width: 500px;position: relative; left: 200px; top: -20px;">
            ...
            <small style="font-size: 10px; color: gray;" id="editTime">${t.editTime}</small></div>
        <div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
    </div>
